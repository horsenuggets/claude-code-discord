...
async function getProcessList(filter?: string, limit: number = 20): Promise<string> {
  try {
    ...
    throw new Error(
      `Failed to get process list: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
async function getDiskUsage(): Promise<string> {
  try {
    ...
    throw new Error(
      `Failed to get disk usage: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
function getEnvironmentVariables(filter?: string): string {
  ...
  if (filter) {
    envEntries = envEntries.filter(([key]) => key.toLowerCase().includes(filter.toLowerCase()));
  }

  if (envEntries.length === 0) {
    return filter
      ? `**No environment variables found matching "${filter}"**`
  ...
  const title = filter
    ? `**Environment Variables (filtered by "${filter}"):**`
    : "**Environment Variables:**";
...
async function getSystemLogs(lines: number = 50, service?: string): Promise<string> {
  try {
    ...
    const title = service
      ? `**System Logs (${service}, last ${lines} lines):**`
      : `**System Logs (last ${lines} lines):**`;
    return `${title}\n\`\`\`\n${output}\n\`\`\``;
  } catch (error) {
...
async function scanPorts(host: string = "localhost", ports?: string): Promise<string> {
  try {
    ...
    const title = ports
      ? `**Open Ports (${host}, filtered by ${ports}):**`
      : `**Open Ports (${host}):**`;
    return `${title}\n\`\`\`\n${filteredOutput}\n\`\`\``;
  } catch (error) {
    throw new Error(
      `Failed to scan ports: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
...
async function getUptimeInfo(): Promise<string> {
  try {
    ...
    throw new Error(
      `Failed to get uptime info: ${error instanceof Error ? error.message : "Unknown error"}`,
    );
  }
}
