...
${systemInfoOutput}`;
    ...
    async onProcesses(ctx: any, filter?: string, limit: number = 20) {
      try {
        ...
        if (platform === "windows") {
          processInfo += `${"Name".padEnd(20)} ${"PID".padEnd(8)} ${"CPU%".padEnd(8)} ${
            "Memory".padEnd(12)
          }\n`;
          processInfo += "-".repeat(50) + "\n";

          limitedProcesses.forEach((proc) => {
            processInfo += `${proc.name.padEnd(20)} ${proc.pid.toString().padEnd(8)} ${
              (proc.cpu || 0).toFixed(1).padEnd(8)
            } ${formatFileSize(proc.memory || 0).padEnd(12)}\n`;
          });
        } else {
          ...
          limitedProcesses.forEach((proc) => {
            processInfo += `${proc.pid.toString().padEnd(8)} ${proc.name.padEnd(20)} ${
              (proc.cpu || 0).toFixed(1).padEnd(8)
            } ${(proc.memory || 0).toFixed(1).padEnd(8)} ${
              (proc.status || "Unknown").padEnd(10)
            }\n`;
          });
        }
    ...
    async onDiskUsage(ctx: any) {
      try {
        ...
        throw new Error(
          `Failed to get disk usage: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
    ...
    async onSystemLogs(ctx: any, lines: number = 50, service?: string) {
      try {
        ...
        if (platform === "windows") {
          if (service) {
            command = [
              "powershell",
              "-Command",
              `Get-EventLog -LogName System -Source "*${service}*" -Newest ${lines} | Select-Object TimeGenerated, Source, EntryType, Message | Format-Table -AutoSize`,
            ];
          } else {
            command = [
              "powershell",
              "-Command",
              `Get-EventLog -LogName System -Newest ${lines} | Select-Object TimeGenerated, Source, EntryType, Message | Format-Table -AutoSize`,
            ];
          }
        } else {
          if (service) {
        ...
        throw new Error(
          `Failed to get system logs: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
    ...
    async onPortScan(ctx: any, host: string = "localhost", ports?: string) {
      try {
        ...
        throw new Error(
          `Failed to scan ports: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
        ...
        if (service) {
          if (platform === "windows") {
            command = [
              "powershell",
              "-Command",
              `Get-Service -Name "*${service}*" | Format-Table -AutoSize`,
            ];
    ...
    async onUptime(ctx: any) {
      try {
        ...
        throw new Error(
          `Failed to get uptime: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    },
  };
