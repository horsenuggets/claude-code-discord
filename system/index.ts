...
${systemInfoOutput}`;
    ...
    async onEnvVars(ctx: any, filter?: string) {
      try {
        ...
        throw new Error(`Failed to get environment variables: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
    ...
    async onSystemLogs(ctx: any, lines: number = 50, service?: string) {
      try {
        ...
        throw new Error(`Failed to get system logs: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
    ...
    async onPortScan(ctx: any, host: string = 'localhost', ports?: string) {
      try {
        ...
        throw new Error(`Failed to scan ports: ${error instanceof Error ? error.message : String(error)}`);
      }
    },
    ...
    async onUptime(ctx: any) {
      try {
        ...
        throw new Error(`Failed to get uptime: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  };
}