...
export async function createClaudeCodeBot(config: BotConfig) {
  ...
  const claudeHandlers = createClaudeHandlers({
    workDir,
    claudeController,
    setClaudeController: (controller) => {
        ...
        await ctx.update({
          embeds: [{
            ...
            fields: [
              {
                name: "Tip",
                value: "Send some Claude commands to build up your message history!",
                inline: false,
              },
            ],
            timestamp: true,
      ...
      await ctx.update({
        embeds: [{
          ...
          fields: [
            {
              name: "Usage",
              value: "Copy this message to use with `/claude prompt:...`",
              inline: false,
            },
            {
              name: "Navigation",
              value: `Position ${historyPosition} of ${totalMessages} messages in history`,
              inline: false,
            },
          ],
          timestamp: true,
    ...
    ["continue", async (ctx: InteractionContext) => {
      ...
      await ctx.update({
        embeds: [{
          ...
          description:
            `Use \`/continue\` or \`/claude session_id:${sessionId}\` to continue the conversation.`,
          fields: [
  ...
  try {
    ...
    if (platform === "windows") {
      ...
      try {
        ...
        console.warn(
          "Could not register SIGBREAK handler:",
          winError instanceof Error ? winError.message : String(winError),
        );
      }
    } else {
      // Unix-like systems
      try {
        ...
        console.warn(
          ...
          unixError instanceof Error ? unixError.message : String(unixError),
        );
      }
    }
  } catch (error) {
    console.warn(
      "Signal handler registration error:",
      error instanceof Error ? error.message : String(error),
    );
  }
