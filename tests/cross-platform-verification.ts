...
async function verifyCrossPlatformSupport() {
  ...
  try {
    ...
    console.log(
      `   ❌ System info command failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    allTestsPassed = false;
  }
  ...
  try {
    ...
    console.log(
      `   ❌ Process list failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    allTestsPassed = false;
  }
  ...
  if (platform === "windows") {
    ...
    if (powershellTest) {
      ...
      try {
        ...
        console.log(
          `   ❌ PowerShell test error: ${error instanceof Error ? error.message : String(error)}`,
        );
        allTestsPassed = false;
      }
    } else {
