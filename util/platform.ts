    ...
    case "windows":
      return {
        ...
        systemLogsCmd: [
          "powershell",
          "-Command",
          "Get-EventLog -LogName System -Newest 50 | Select-Object TimeGenerated, Source, EntryType, Message | Format-Table -AutoSize",
        ],
        serviceStatusCmd: ["powershell", "-Command", "Get-Service | Format-Table -AutoSize"],
        uptimeCmd: [
          "powershell",
          "-Command",
          "(Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime; (Get-Date) - (Get-CimInstance -ClassName Win32_OperatingSystem).LastBootUpTime",
        ],
...
export async function isCommandAvailable(command: string): Promise<boolean> {
  try {
    const platform = detectPlatform();
    const checkCmd = platform === "windows" ? ["where", command] : ["which", command];
